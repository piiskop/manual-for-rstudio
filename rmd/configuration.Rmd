...
As PDF-file is generated according to \TeX file, it's sometimes useful^[asdföljka] to see what \TeX file consists of.
...
Further one in the same block of *knitr* options, there's hooking that creates line numbers for source code blocks as comments at the end of each line as described in @riederer_122. This hook is necessary as `attr.source = '.numberLines'` doesn't work for *bs4_book* and i don't like the layout of *gitbook* where that particular thing works.

Next, `librarian` is being loaded because i don't want to check manually if a particular package exists.

Finally, there are the definitions of two helper functions: for including graphics and creating tables.
...
This file contains ECMAScript script that must be included in the header of the webpage.
...
workflow: r -> bookdown -> pandoc -> ...

tabel \@ref(tab:tableofnoting)

```{r label = "tableofnoting", attr.source='.numberLines'}
a <- c(2, 3, 4)
b <- data.frame(a)
cas <- function(asdf, amber, heard, still, does, need, to, fail, indication, has, decided, pay, please, plea, ask, look, document, seventeen, october, sixteen) {
  return("sa")
}
print_table(a, caption = "flökj")
```


joonis \@ref(fig:dia)

```{r label = "dia", fig.cap = "asdf."}
