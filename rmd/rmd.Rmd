...
For inline referencing, wherever i want to have a reference to that reference record i call `render_with_emoji` and feed it with a value for `text` as the identifier of the internal reference in parenthesis if the reference is direct and in double parenthesis if indirect:
...
The same equation inline: $\frac{t_\text{d}}{\unit{\degreeCelsius}}$.
