...
`\begin{multicols}{2}` does not allow markdown's sharp symbol. In order to tolerate captions inside a multi-column environment, the command must be redefined and used as a command without any parameters (listing \@ref(lst:multicolsbegin) on the page `r render_pageref(reference="lst:multicolsbegin")`)[@cant].
...
\begin{itemize}
	...
	\item[\faSquareO] rubiinide punane värvus on tingitud kroomiioonide
	  olemasolust;
...
An example of how to force a line break is shown in the listing \@ref(lst:example-of-linebreak) on the page `r render_pageref(reference="lst:example-of-linebreak")`[@kottwitz_latex, lk 271].
...
(ref:explanation-for-itemize) For creating tasks for pupils, sometimes checkboxes are needed. For that, there is `itemize` with the package `fontawesome`[@roy_2019_itemize]. An example with `itemize` is shown in the listing \@ref(lst:itemize) on the page `r render_pageref(reference="lst:itemize")`.
