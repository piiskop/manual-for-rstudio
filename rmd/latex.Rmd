...
& C_{10} && := -4.8640239 \times 10^{-2},\\
& C_{11} && := 4.1764768 \times 10^{-5},\\
& C_{12} && := -1.4452093 \times 10^{-8} % (\#eq:saturation-pressure-over-liquid-h2o)
\label{eq:saturation-pressure-over-liquid-h2o}
% (\#eq:saturation-pressure-over-liquid-h2o)
\end{align}
...
Referencing an equation is also visible in the equation \@ref(eq:saturation-pressure-over-liquid-h2o):

```{verbatim label = "ref"}
\@ref(eq:saturation-pressure-over-liquid-h2o)

```

The label must be placed to the end of the row, not to a separate row which would still work for PDF but not for HTML output. Additional labels must be placed in the end of the corresponding row just before the double slashes before the line break.
